...
export async function listProfiles(tnId: number, opts: ListProfilesOptions) {
	...
	const q =
		`SELECT * FROM profiles WHERE tnId = $tnId AND idTag != $ownIdTag` +
		(opts.status ? ` AND status IN (${opts.status.map(ql).join(',')})` : '') +
		(opts.type
			? opts.type === 'person'
				? ' AND type ISNULL'
				: ` AND type = ${ql(opts.type == 'community' ? 'C' : '')}`
			: '') +
	...
	const res: Profile[] = rows.map((row) => ({
		...
		connected: row.connected == null ? undefined : !row.connected ? 'R' : !!row.connected
	}))
...
export async function readProfile(tnId: number, idTag: string) {
	...
	const ret: Profile = {
		...
		connected:
			profile.connected === null ? undefined : !profile.connected ? 'R' : !!profile.connected
	}
	return ret
}
...
export async function createProfile(
	...
	await db.run(
		`INSERT INTO profiles (tnId, idTag, name, type, profilePic, eTag, status)
...
export async function updateProfile(tnId: number, idTag: string, opts: UpdateProfileOptions) {
	...
	if (opts.connected !== undefined)
		update.push(`connected = ${ql(opts.connected == 'R' ? false : opts.connected)}`)
...
export async function addProfilePublicKey(
	tnId: number,
	idTag: string,
	key: { keyId: string; publicKey: string; expires?: number }
) {
	await db.run(
		'INSERT OR REPLACE INTO key_cache (idTag, keyId, publicKey) VALUES ($idTag, $keyId, $publicKey)',
		{
			$idTag: idTag,
			$keyId: key.keyId,
...
export async function processProfileRefresh(
	...
	for (const profile of profiles) {
		...
		try {
			...
			if (val) {
				await db.run(
					'UPDATE profiles SET syncedAt=unixepoch() WHERE tnId = $tnId AND idTag = $idTag',
					{
						$tnId: profile.tnId,
						$idTag: profile.idTag
					}
