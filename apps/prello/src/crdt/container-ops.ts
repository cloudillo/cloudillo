...
import type { YPrelloDocument, ChildRef, StoredContainer, StoredObject } from './stored-types'
import type { ObjectId, ContainerId } from './ids'
import { generateContainerId, toObjectId, toContainerId } from './ids'
import type { ContainerNode, ContainerType, PrelloObject } from './runtime-types'
...
export function getContainerChildrenData(
	doc: YPrelloDocument,
	containerId: ContainerId
): Array<{ type: 'object' | 'container'; data: PrelloObject | ContainerNode }> {
	const children = doc.ch.get(containerId)
	if (!children) return []
