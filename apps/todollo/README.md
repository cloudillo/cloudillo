# Todollo - Real-time Todo List Tutorial

**A simple tutorial app demonstrating Cloudillo's real-time database (RTDB).**

Todollo is a minimalist collaborative todo list that shows you how to build real-time applications with Cloudillo. When one user creates, completes, or deletes a task, all connected users see the change instantly.
...
const unsubscribe = client.collection('tasks').onSnapshot(
  (snapshot) => {
...
### 1. Initialization (useTodollo hook in app.tsx)
...
const token = await cloudillo.init('todollo')
...
- **TodolloApp**: Main component that ties everything together
