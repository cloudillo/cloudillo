	...
	React.useEffect(() => {
		...
		;(async () => {
			try {
				...
				console.log('[Todollo] Initializing cloudillo...')
				const token = await cloudillo.init(APP_NAME)
				console.log('[Todollo] Token received')
				...
				console.log('[Todollo] Connecting to RTDB...')
				...
				console.log('[Todollo] Connected to RTDB')
				setClient(rtdbClient)
				setConnected(true)
				setLoading(false)
			} catch (err) {
				console.error('[Todollo] Initialization error:', err)
				if (!unmounted) {
...
function Header({
	...
	return (
		<div className="todollo-header">
			<h1 className="header-title">Todollo</h1>
 ...
 *
 * 1. Initialize connection (useTodollo hook)
...
export function TodolloApp() {
	...
	if (todollo.loading) {
		return (
			<div className="c-vbox w-100 h-100 justify-center align-center">
				...
				<p className="mt-2">Connecting to Todollo...</p>
			</div>
		)
	}
	...
	return (
		<div className="todollo-app c-vbox w-100 h-100">
			<Header
				connected={todollo.connected}
