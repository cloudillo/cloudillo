...
/* Profile */
/***********/
export function UnknownProfilePicture({ small, tiny }: { small?: boolean, tiny?: boolean }) {
	return <svg className={'picture' + (tiny ? ' tiny' : small ? ' small' : '')} viewBox="0 0 24 24" fill="none">
...
export function ProfilePicture({ className, profile, small, tiny, srcTag }: { className?: string, profile: { profilePic?: string }, small?: boolean, tiny?: boolean, srcTag?: string }) {
	const [auth] = useAuth()
