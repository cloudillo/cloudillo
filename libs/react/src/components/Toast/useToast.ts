...
export function useToast(): UseToastReturn {
	...
	const success = React.useCallback((message: string, options?: Omit<ToastOptions, 'variant'>) => {
		return toast({ ...options, message, variant: 'success' })
	}, [toast])

	const error = React.useCallback((message: string, options?: Omit<ToastOptions, 'variant'>) => {
		return toast({ ...options, message, variant: 'error' })
	}, [toast])

	const warning = React.useCallback((message: string, options?: Omit<ToastOptions, 'variant'>) => {
		return toast({ ...options, message, variant: 'warning' })
	}, [toast])

	const info = React.useCallback((message: string, options?: Omit<ToastOptions, 'variant'>) => {
