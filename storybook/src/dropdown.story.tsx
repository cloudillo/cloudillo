...
export function DropdownStory() {
	return <Story
		...
		props={[
			...
			{ name: 'triggerClassName', type: 'string', descr: 'CSS classes for trigger element' },
			{ name: 'menuClassName', type: 'string', descr: 'CSS classes for menu container' },
			{ name: 'elevation', type: '"low" | "mid" | "high"', descr: 'Menu elevation level' },
			{ name: 'emph', type: 'boolean', descr: 'Emphasized menu style' },
			{ name: 'placement', type: '"bottom-start" | "bottom-end" | "top-start" | "top-end"', descr: 'Menu placement' }
		]}
	>
		<Variant name="Basic Dropdown">
			<Dropdown
				trigger={<span className="c-nav-item g-2">Options <IcChevron/></span>}
				emph
			>
				<NavItem gap={2}><IcEdit/> Edit</NavItem>
				<NavItem gap={2}><IcCopy/> Duplicate</NavItem>
				<NavItem gap={2}><IcShare/> Share</NavItem>
				<NavItem gap={2}><IcDelete/> Delete</NavItem>
			</Dropdown>
		</Variant>

		<Variant name="Icon Only Trigger">
			<Dropdown
				trigger={<span className="c-nav-item"><IcMore/></span>}
				emph
			>
		...
		<Variant name="With Elevation">
			<div className="c-hbox g-3">
				<Dropdown
					trigger={<span className="c-button">Low</span>}
					elevation="low"
				>
					<NavItem>Low elevation menu</NavItem>
				</Dropdown>
				<Dropdown
					trigger={<span className="c-button">High</span>}
					elevation="high"
				>
