...
export function SelectStory() {
	...
	return <Story
		...
		props={[
			...
			{ name: 'getData', type: '(q: string) => Promise<T[]>', descr: 'Async function to fetch data', required: true },
			{ name: 'onChange', type: '(item: T) => void', descr: 'Called when item is selected' },
			{ name: 'onSelectItem', type: '(item: T | undefined) => void', descr: 'Called when selection changes' },
			{ name: 'itemToId', type: '(item: T) => string', descr: 'Extract unique ID from item', required: true },
			{ name: 'itemToString', type: '(item: T | null) => string', descr: 'Convert item to string', required: true },
			{ name: 'renderItem', type: '(props: T) => React.ReactNode', descr: 'Render function for dropdown items', required: true }
		]}
	>
				...
				<Select<User>
					...
					renderItem={item => (
						<div>
							...
							<div style={{ fontSize: '0.85em', color: '#666' }}>{item.email}</div>
						</div>
					)}
				/>
				{selectedUser && (
