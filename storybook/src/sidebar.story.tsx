...
function SidebarDemo() {
	...
	return (
		<div className="pos-relative" style={{ height: '400px', border: '1px solid var(--col-border)' }}>
			...
			<Sidebar sidebar={sidebar} side="left">
				<SidebarContent>
					...
					<SidebarNav>
						<NavItem gap={2}><IcHome/> Home</NavItem>
						<NavItem gap={2}><IcUser/> Profile</NavItem>
						<NavItem gap={2}><IcFolder/> Files</NavItem>
					</SidebarNav>
					<SidebarSection title="Settings">
						<SidebarNav>
							<NavItem gap={2}><IcSettings/> Preferences</NavItem>
						</SidebarNav>
					</SidebarSection>
			...
			<div className="p-3" style={{ marginLeft: sidebar.isPinned ? '240px' : 0, transition: 'margin 0.3s' }}>
				<HBox gap={2} className="mb-3">
					<Button onClick={sidebar.toggle}><IcMenu/></Button>
					<span>Click the menu button to toggle sidebar</span>
				</HBox>
				<p>Status: {sidebar.isOpen ? 'Open' : 'Closed'}, {sidebar.isPinned ? 'Pinned' : 'Unpinned'}</p>
				<p>Main content area. The sidebar can overlay this content or push it aside when pinned.</p>
			</div>
		</div>
	)
}
...
export function SidebarStory() {
	return <Story
		...
		props={[
			...
			{ name: 'pinned', type: 'boolean', descr: 'Whether sidebar is pinned (pushes content)' },
...
// - setWidth(width)`}</pre>
		...
		<Variant name="Sidebar Components">
			<div className="c-panel p-3">
				<ul>
					<li><code>Sidebar</code> - Main container</li>
					<li><code>SidebarContent</code> - Content wrapper</li>
					<li><code>SidebarHeader</code> - Header section</li>
					<li><code>SidebarFooter</code> - Footer section</li>
					<li><code>SidebarNav</code> - Navigation list</li>
					<li><code>SidebarSection</code> - Grouped section with title</li>
					<li><code>SidebarBackdrop</code> - Overlay backdrop</li>
					<li><code>SidebarToggle</code> - Toggle button</li>
					<li><code>SidebarResizeHandle</code> - Drag to resize</li>
				</ul>
			</div>
		</Variant>
	</Story>
