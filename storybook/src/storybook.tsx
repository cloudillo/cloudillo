	...
	return (
		<div className="sb--m-page">
			<nav className="sb--m-nav c-panel">
				<ul>
					{Object.entries(registry).map(([path, node]) => (
						<li key={path}>
...
function StoryPropType({
	...
	if (typeof type == 'string') {
		...
		return (
			<>
				{type.map((t, idx) => (
					<StoryPropType key={idx} type={t} indentDepth={(indentDepth || 0) + 1} />
				))}
			</>
		)
	}
...
export function Story({ name, path, title, description, props, children }: StoryProps) {
	...
	return (
		<div className="sb--m-story">
			<h2>{title ?? name}</h2>
			<p className="description">{description}</p>
			{props && (
				<table className="prop-list">
					<tbody>
						{props.map((prop) => (
							<tr key={prop.name}>
								<td className="name">
									<b>
										{prop.name}
										{prop.required && <span style={{ color: '#f88' }}>*</span>}
									</b>
								</td>
								<td className="type">
									<StoryPropType type={prop.type} />
								</td>
...
export function Variant({
	...
	return (
		<div className="sb--m-variant">
			<h3>{name}</h3>
			<p className="description">{description}</p>
			{children}
			{code && (
				<div className={'code' + (more ? ' more' : '')}>
					<pre style={{ whiteSpace: 'pre-wrap' }}>{code}</pre>
					<button className="a-btn small" onClick={() => setMore(!more)}>
